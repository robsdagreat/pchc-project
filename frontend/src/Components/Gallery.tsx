...
const ImageThumbnail: React.FC<{ image: GalleryDataItem; onClick: () => void; index: number }> = ({ image, onClick, index }) => {
...
const Gallery: React.FC = () => {
  ...
  const categories = useMemo(() => {
    const cats = ['All', ...Array.from(new Set(galleryData.map(img => img.category).filter(Boolean)))];
  ...
  const handleThumbnailClick = (image: GalleryDataItem) => {
    const index = galleryData.findIndex(img => img.id === image.id);
